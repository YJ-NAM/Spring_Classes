...
/*
 * 스프링 컨테이너의 종류
 * - BeanFactory : BeanFactory 계열의 인터페이스만 구현한 클래스는 단순히 스프링 컨테이너에서 
 *                 객체를 생성하고 DI만 처리해 주는 기능만을 제공해 주는 객체.
 ...
 *                 다양한 부가 기능(트랜잭션 처리, 자바 코드 기반의 스프링 설정, 애노테이션을 
 *                 이용한 빈 설정, 스프링을 이용한 웹 개발 등등) 때문인데 이러한 부가 기능을 사용하기 
 *                 위해서는 ApplicationContext 객체를 주로 사용하게 됨.
 * - ApplicationContext : Bean을 등록, 생성, 조회, 반환 관리하는 기능은 BeanFactory와 같음.
 ...
 *                                  읽어오는 역할을 함. GemericXmlApplicationContext
 *                                  객체를 생성할 때 파라미터 값으로 "classpath:mydao.xml"
 *                                  을 전달했는데 이는 클래스 패스에 위치한 xml 파일을 설정 
 *                                  파일로 사용한다는 의미임.
...
public class Main {
	...
	public static void main(String[] args) {
		...
		// -> 여기서만 객체 생성함
		// xml 파일을 이용하여 메모리로 해당 xml 파일이 로딩됨
